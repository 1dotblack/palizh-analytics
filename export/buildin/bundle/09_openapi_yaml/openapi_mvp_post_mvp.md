...
<!-- buildin-source: Техническая часть/openapi_mvp_post_mvp.yaml -->

```yaml
---
openapi: 3.1.0
info:
  title: Palizh MVP API
  version: 0.1.0
  summary: Post-MVP API — auth, catalog, cart, checkout, orders, claims, documents, admin
  description: >
    OpenAPI Palizh B2B (post-MVP): onboarding, токены, профиль, каталог, корзина, заказы, претензии,
    документы, админка, обучение (без обмена с 1С по обучению), и **вход 1С → платформа** —
    единый `POST /exchange` (см. `openapi_1c_inbound_mvp.yaml`, HMAC). Вызовы **платформа → 1С** —
    `1c-http-openapi.yaml` (`1C_API_контракт_и_этапы.md`).
servers:
...
- name: Documents
- name: Training
  description: 'Обучение (программы, заявки): контент и заявки **только на платформе** (админка / CMS). **Обмена с 1С нет.**

'
- name: Admin
- name: Integration 1C
  description: 'Server-to-server: POST /exchange (1С → платформу), event + payload. HMAC. См. openapi_1c_inbound_mvp.yaml.'
paths:
  ...
  "/search/products":
    get:
      ...
      description: >
        Same visibility rules as catalog listing: `exclusiveCounterpartyGuid` is evaluated on **base nomenclature** (aligned
        with `Product` in `1c-http-openapi.yaml`). Search results **never** include products that are not visible to the current
        viewer (guest / other B2B).
...
'
  ...
  "/claims/{claimId}/attachments":
    post:
      ...
      operationId: uploadClaimAttachment
      security:
      - bearerAuth: []
      parameters:
      - name: claimId
        ...
        '404':
          ...
          content:
            application/json:
              schema:
                "$ref": "#/components/schemas/ErrorResponse"
  "/reconciliation-acts/request":
    post:
      tags:
      - Documents
      summary: Request reconciliation act
      operationId: requestReconciliationAct
      security:
      - bearerAuth: []
      requestBody:
        required: true
        content:
          application/json:
            schema:
              "$ref": "#/components/schemas/ReconciliationActRequest"
      responses:
        '202':
          description: Reconciliation act request accepted
          content:
            application/json:
              schema:
                "$ref": "#/components/schemas/DocumentRequestResponse"
  ...
  "/admin/applications":
    get:
      tags:
      - Admin
      summary: List incoming applications
      description: Requires admin role
  ...
  "/public/training-programs":
    get:
      tags:
      - Training
      summary: List training programs (public)
      description: 'Content from platform admin / CMS; not from 1C.

'
      operationId: listPublicTrainingPrograms
      security:
      - {}
      - bearerAuth: []
      parameters:
      ...
      responses:
        '200':
          description: Training program list
          content:
            application/json:
              schema:
                "$ref": "#/components/schemas/TrainingProgramListResponse"
  "/public/training-programs/{programId}":
    get:
      tags:
      - Training
      summary: Get training program details (public)
      description: 'Content from platform; not from 1C.

'
      operationId: getPublicTrainingProgram
      security:
      - {}
      - bearerAuth: []
      parameters:
  ...
  "/training-applications":
    post:
      tags:
      - Training
      summary: Submit training application (authorized only)
      description: >
        Creates a training application on the platform (status `sent`) and triggers an internal email notification to recipients
        configured in admin routing for the application type. **Not** sent to 1C.
      operationId: createTrainingApplication
      security:
      - bearerAuth: []
      requestBody:
        required: true
        content:
          application/json:
            schema:
              "$ref": "#/components/schemas/TrainingApplicationCreateRequest"
      responses:
        '201':
          description: Training application created
          content:
            application/json:
              schema:
                "$ref": "#/components/schemas/TrainingApplicationCreateResponse"
        '401':
          description: Unauthorized
          content:
            application/json:
        ...
        '422':
          ...
          content:
            application/json:
              schema:
                "$ref": "#/components/schemas/ValidationErrorResponse"
  "/me/training-applications":
    get:
      tags:
      - Training
      summary: List current user's training applications
      description: 'Platform-only history; not from 1C.

'
      operationId: listMyTrainingApplications
      security:
      - bearerAuth: []
      responses:
        '200':
          description: Training application list
          content:
            application/json:
              schema:
                "$ref": "#/components/schemas/MyTrainingApplicationListResponse"
        '401':
          description: Unauthorized
          content:
            application/json:
...
components:
  ...
  schemas:
    ...
    ProductAttribute:
      ...
      additionalProperties: false
      description: >
        Attribute value on a product in catalog API responses: stable `code`, human-readable `name`, and `value`.
        Facet-фильтры каталога на стенде — **`GET /products/filters`** (`public-http-openapi.yaml`), не отдельный
        `GET /product-attributes`. Optional `valueType` documents the kind of value for UI/analytics.
      required:
      - code
      - name
      - value
    ...
    ProductPrice:
      ...
      additionalProperties: false
      description: |
        Цена товара в текущем MVP API — одна цена за единицу измерения для авторизованного B2B-клиента.
        Модель «двух цен» (кратно / некратно упаковке) из ЧТЗ 09 §3.2 и `НаборУпаковок` 1С
        здесь НЕ моделируется — это следующий слой API (post-MVP). Корзинные пересчёты
        по кратности и количественные скидки также не входят в этот объект.
      required:
      - currency
      - amount
      properties:
...
'
    ...
    ClaimCreateRequest:
      ...
      properties:
        ...
        extra:
          "$ref": "#/components/schemas/ClaimExtra"
    ClaimCreateResponse:
      type: object
      additionalProperties: false
      required:
    ...
    ClaimDetailResponse:
      ...
      properties:
        ...
        attachments:
          type: array
          items:
            "$ref": "#/components/schemas/ClaimAttachmentMeta"
    ClaimItem:
      type: object
      additionalProperties: false
      required:
    ...
    ClaimExtra:
      ...
      description: >
        Type-specific extra fields depending on claimType. For damage: damageDetectedOnReceipt, hasCarrierAct, carrierActNumber,
        damageDescription. For shortage: shortageDetectedOnReceipt, packagesComment. For surplus: surplusLocation, surplusCondition.
        For quality: qualityIssueType, qualityDetectedAt, usageConditions, needsLabMaterials.
    ...
    TrainingProgramListResponse:
      ...
      properties:
        items:
          type: array
          items:
            "$ref": "#/components/schemas/TrainingProgramSummary"
    ...
    TrainingApplicationCreateResponse:
      ...
      properties:
        applicationId:
          ...
          format: uuid
        status:
          "$ref": "#/components/schemas/TrainingApplicationStatus"
        message:
          type: string
    MyTrainingApplicationItem:
      type: object
      additionalProperties: false
      required:
      - applicationId
      ...
      properties:
        ...
        payload:
          "$ref": "#/components/schemas/TrainingApplicationCreateRequest"
    MyTrainingApplicationListResponse:
      type: object
      additionalProperties: false
      required:
      - items
      properties:
        items:
          type: array
          items:
            "$ref": "#/components/schemas/MyTrainingApplicationItem"
    OneCExchangeRequest:
      type: object
      additionalProperties: false
      required:
      ...
      properties:
        event:
          type: string
          description: >
            Код сценария. Зафиксировано (тимлид, MVP): `sync.products`, `sync.counterparties`, `sync.categories`. Расширение
            перечня — по согласованию (статусы заказа, документы и т.д.).
          enum:
          - sync.products
          - sync.counterparties
          - sync.categories
        payload:
          description: |
            Данные события: **массив** (типично для трёх сценариев выше) или **объект** — по согласованию.
            Состав полей payload для `sync.*` — см. `входящие/incoming-hooks.md` (примеры товаров, контрагентов, категорий).
          oneOf:
          - type: array
            items: {}
          - type: object
