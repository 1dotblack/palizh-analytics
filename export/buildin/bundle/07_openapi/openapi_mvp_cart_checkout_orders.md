...
> **MVP / стенд (2026-07-09):** канон корзины, заказов и ЛК — **`public-http-openapi.yaml`** (`GET/PUT /cart`, `POST /orders`, `GET /orders`, mixed qty в `CartItem`). Этот документ описывает черновик `openapi_client_mvp.yaml`. См. [`OpenAPI_индекс.md`](OpenAPI_индекс.md), [`Задание_создание_заказа_MVP.md`](Задание_создание_заказа_MVP.md), [`Приёмка_корзины_vs_счёт_1С.md`](Приёмка_корзины_vs_счёт_1С.md).
...
**Канон ответа:** [`public-http-openapi.yaml`](public-http-openapi.yaml) → `CartItem` (как в коде `CartItemResource`).  
**Не входят в API:** `packagingMode`, `fullBoxesCount`, `remainderQuantity`, `referenceUnitPrice*`, `warnings` — внутренние величины формулы / UI на фронте.

**Алгоритм `lineTotal`** (`Product::calculateLineTotal`):

1. Нет/`0` `unitsPerBox`, нет цены коробки, или `qty < unitsPerBox` → `qty × pricePerPiece`.
2. Иначе → `fullBoxes × pricePerBox + remaining × pricePerPiece`.

```
pricePerBox   = discountedAmountPerBox ?? amountPerBox
pricePerPiece = discountedUnitPrice ?? unitPrice
```

| Поле `CartItem` | Смысл |
| --------------- | ----- |
| `unitPrice` | Цена **за штуку** (базовая из прайса), не средняя |
| `discountedUnitPrice` | Цена за штуку со скидкой (если есть) |
| `unitsPerBox` | Из товара 1С |
| `amountPerBox` / `discountedAmountPerBox` | Цена за коробку |
| `lineTotal` | Сумма строки (авторитет для UI и итогов) |

| Итог корзины | Смысл |
| ------------ | ----- |
| Σ `lineTotal` | Сумма позиций |

**Пример:** `unitsPerBox = 15`, `qty = 20` → `1 × pricePerBox + 5 × pricePerPiece`.

**Модель:** вариант **A** (backend). Подробности — **ЧТЗ 01 §4.2.4**.
