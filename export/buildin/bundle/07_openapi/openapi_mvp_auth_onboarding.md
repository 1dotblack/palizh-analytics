...
> **MVP / стенд (2026-07-09):** канон публичного HTTP API — **`public-http-openapi.yaml`** (Laravel Sanctum, cookie/CSRF). Этот документ поясняет **устаревшую для стенда** модель `openapi_client_mvp.yaml` (Bearer/JWT). См. [`OpenAPI_индекс.md`](OpenAPI_индекс.md).
