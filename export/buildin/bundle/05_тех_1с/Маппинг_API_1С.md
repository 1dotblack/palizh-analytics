<!-- buildin-source: Техническая часть/Маппинг_API_1С.md -->
...
| unitPrice | number | да | — | — | — | Цена **за штуку** из прайса (`amount` / snapshot); **не** средняя `lineTotal/qty` | — |
| lineTotal | number | да | — | — | — | `Product::calculateLineTotal` (коробки × `amountPerBox` + остаток × цена шт.) | — |
| unitsPerBox | integer ǀ null | нет | Справочник.Номенклатура | НаборУпаковок | Число | Из `sync.products` | ≈ |
| amountPerBox | number ǀ null | нет | — | — | — | Цена за коробку из `sync.prices` | ≈ |
| discountedUnitPrice | number ǀ null | нет | — | — | — | Персональная цена за штуку | ≈ |
| discountedAmountPerBox | number ǀ null | нет | — | — | — | Персональная цена за коробку | ≈ |
...
| -- | -- | -- | -- | -- | -- | -- | -- |
| id | string | да | — | — | — | Внутренний ID платформы | — |
| guid / externalGuid | string (uuid) | да | Документ.ЗаказКлиента | Ref | GUID | **Ключ связи:** GUID заказа на платформе = GUID документа в 1С (`externalGuid` в `post_orders`) | ✓ |
| number | string | да | Документ.ЗаказКлиента | Номер | Строка | Из ответа `post_orders.orderNumber` | ✓ |
...
| -- | -- | -- | -- | -- |
| pending | Заказ создан на платформе, ожидается подтверждение из 1С | Платформа | `POST /orders` выполнен, `post_orders` ещё не успешен | ≈ |
| synced | Заказ принят 1С; GUID уже известен как `externalGuid` | Платформа + 1С | Получен `orderNumber`, sync подтверждён | ≈ |
...
- (Архив) [Задание_1С_разработчику.md](../архив/Задание_1С_разработчику.md) — снято с поддержки (2026-07-09); актуальны [ЧТЗ/09_интеграция_1С.md](../ЧТЗ/09_интеграция_1С.md), [Интеграция_1С.md](Интеграция_1С.md), чеклисты `Чеклист_1С_sync.*.md`
- (Архив) [Задание_разработчику_админка_MVP.md](../архив/Задание_разработчику_админка_MVP.md) — снято с поддержки (2026-07-09); актуальны [ЧТЗ/12_система_управления.md](../ЧТЗ/12_система_управления.md), [admin_role_and_routing_matrix.md](admin_role_and_routing_matrix.md)
