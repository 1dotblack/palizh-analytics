<!-- buildin-source: Техническая часть/Принятые_решения_API_интеграция_1С.md -->
...
| Решение | Содержание |
|--------|------------|
| **Создание заказа** | `POST /post_orders`. Тело: `OrderCreateRequest` — `externalGuid` (= GUID заказа на платформе **и** GUID документа в 1С), `productGuid` + `productCharacteristicGuid` в позициях, цены string. Ответ **201**: `{ orderNumber }` — GUID в ответе **не** возвращается. Канон: `1c-http-openapi.yaml`. |
...
| **Ключ связи заказа (2026-08-07)** | `externalGuid` (платформа `orders.guid`) = GUID документа «Заказ клиента» в 1С. 1С **создаёт** заказ с этим GUID; в ответе `post_orders` GUID **не** дублируется — только `orderNumber`. В `sync.documents` при `relatedEntityType: order` поле `relatedEntityGuid` = этот же GUID (платформа: `Order::where('guid', …)`). Fallback «GUID только из ответа 1С» — только если создание с заданным GUID в ERP невозможно. |
...
| **Формула строки корзины** | `lineTotal` по `Product::calculateLineTotal`: при `qty ≥ unitsPerBox` и наличии цены коробки — `fullBoxes × pricePerBox + remaining × pricePerPiece`; иначе все штуки × цена за штуку. В API: `unitPrice` = цена **за штуку** (не средняя); `lineTotal` — авторитет суммы. Поля `packagingMode*` **не отдаются**. **ЧТЗ 01 §4.2.4** (уточнено 2026-08-07). |
