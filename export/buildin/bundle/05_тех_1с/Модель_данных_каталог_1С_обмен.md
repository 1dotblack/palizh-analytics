<!-- buildin-source: Техническая часть/Модель_данных_каталог_1С_обмен.md -->
...
- Платформа собирает **SKU/вариант для заказа** по связке **`productGuid` + `characteristicGuid`** (в исходящем `POST /post_orders` — поля **`productGuid`** + **`productCharacteristicGuid`**).
