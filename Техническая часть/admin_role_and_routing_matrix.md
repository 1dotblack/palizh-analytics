...
- `MVP` админки не заменяет `1С`, CRM и внутренние процессы, а покрывает:
  ...
  - настройку маршрутизации внутренних обращений;
  - просмотр служебного списка входящих обращений.
...
- Для `MVP` жизненный цикл претензий и большинства обращений ведётся вне платформы; админка хранит факт поступления, адресатов и служебный мониторинг.
...
| Просмотр списка обращений / заявок | Да | Да | Ограниченно, только по своим контентным сценариям при необходимости |
...
| Список обращений | Мониторинг заявок и обращений | Полноценный workflow обработки post-`MVP` |
...
| Общая обратная связь / чат | Витрина и ЛК | Да | Менеджер по сопровождению |
| Претензия | ЛК | Да | Отдел качества / внутренний контур |
| Заявка по оборудованию / нестандартному заказу | ЛК / форма связи | Да | Менеджер / региональный менеджер |
...
| ------------- | -------------------- | --------------------------- | --------------------- | ------------------------------ |
| `Стать клиентом` | Данные заявки, `GUID`, контакты, дата подачи | Список адресатов / канал, шаблон внутреннего уведомления | Менеджер по сопровождению, далее ручная работа в `1С` | Факт подачи; дальше доступ открывается после активации |
| Общая обратная связь | Текст обращения, контакты, дата | Адресаты / канал | Менеджер по сопровождению / CRM | В `MVP` отдельный workflow не обязателен |
| Претензия | Карточка претензии, вложения, связанный заказ / отгрузка | Адресаты / канал для внутренних уведомлений | Отдел качества / `Mass Project` / почта / CRM | `Отправлена` |
| Заявка по оборудованию / нестандартному заказу | Тип заявки, комментарий, вложения | Адресаты / канал | Менеджер / региональный менеджер / `1С` по факту обработки | В `MVP` служебный список без сложного жизненного цикла |
| Заявка на обучение | Карточка заявки / регистрация на событие | Адресаты / канал | Команда обучения / маркетинг | В `MVP` служебный список без сложного жизненного цикла |
| Запрос документа | Тип документа, связанный заказ / период | Адресаты / канал, если нужен ручной контроль | Менеджер / `1С` / внешняя отправка | Зависит от сценария документа |
...
- приоритетный канал, если после `MVP` появятся дополнительные каналы.
...
| Настройки `SMS` / `push` | Ограниченно / позже | По мере появления канала |
