...
**Импорт каталога из 1С на платформу** (full/delta, server-to-server) — в [`openapi_mvp_integration_1c.md`](openapi_mvp_integration_1c.md) и путях `/integrations/1c/*` в [`openapi_mvp.yaml`](openapi_mvp.yaml).

---

## 1. Базовые решения

- **Интеграция 1С ↔ платформа (2026-04-21):** первая выгрузка каталога на платформу — **полная**; далее — **только дельты**. Первичное заполнение категорий и номенклатуры — разовая **входящая** загрузка на платформу (`POST /integrations/1c/catalog/sync` в OpenAPI). Публичные `GET` ниже — для витрины и ЛК; **не** дублируют протокол импорта из 1С.
...
- В этом контракте цена **не моделирует корзинные пересчёты, количественные скидки и двойные цены кратно/некратно упаковке**. Модель двух цен (цена за единицу в упаковке vs цена за единицу вне упаковки) и `НаборУпаковок` из 1С — см. [ЧТЗ/09_интеграция_1С.md §3.2](../ЧТЗ/09_интеграция_1С.md). Это **следующий API-слой** (post-MVP).
