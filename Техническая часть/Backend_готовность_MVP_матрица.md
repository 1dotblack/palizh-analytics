...
**Сводный аудит:** [`Аудит_готовности_2026-07-06.md`](../Аудит_готовности_2026-07-06.md) (бэк + фронт, E2E).
...
**Итого по бэкенду (API + Filament): ~65–70%.** Публичный HTTP-контур и админка сильные; **критический путь MVP** упирается в **B1–B6** (1С outbound/inbound, корзина mixed qty, документы). Сквозной E2E «корзина → 1С → статусы → документы» на стенде **не замкнут** (~10% вместе с фронтом — см. аудит).

| Область (агрегат) | Оценка | Комментарий |
|-------------------|--------|-------------|
| Auth, каталог, поиск, претензии API | **~80–85%** | Готово к стыковке с фронтом |
| Заказы на платформе (без живой 1С) | **~70%** | CRUD, repeat; `SendOrderJob` — заглушка |
| Интеграция 1С (in + out) | **~15–25%** | Каталог inbound ~60%; outbound и статусы — нет |
| Корзина по ЧТЗ 01 §4.2.4 | **~30%** | API есть; расчёт строки **неверен** (B5) |
...
| Корзина `CartItem` | `unitPrice`, `lineTotal` = `unit_price × qty` | `packagingMode`, `fullBoxesCount`, `remainderQuantity` (ЧТЗ 01 §4.2.4) | **Блокер реализации** — см. §1а B5, §5 B5 |

---

## 1а. Сделано неверно (код ↔ ЧТЗ)

Расхождения, которые **нельзя** закрыть только документацией — нужна правка кода на стенде.

| ID | Проблема | Факт в коде (`f497379`) | Требование (ЧТЗ / спека) | Где в коде |
|----|----------|-------------------------|--------------------------|------------|
| **B5** | Расчёт корзины | `CartService::setQty`: `unit_price = $product->getPrice(...)`; `getPrice()` возвращает только поле **`amount`** (цена за штуку). Итог: `SUM(qty * unit_price)` | ЧТЗ 01 §4.2.4: mixed qty — `fullBoxesCount`, `remainderQuantity`, `amountPerBox`; `lineTotal` по смешанной формуле | `CartService.php`, `Product::getPrice()`, `CartItemResource` |
| **B5** | Ответ API корзины | `CartItemResource`: `lineTotal = unit_price × qty`; нет `packagingMode`, `fullBoxesCount`, `remainderQuantity` | `public-http-openapi.yaml` — схема `CartItem` с полями упаковки | `CartItemResource.php` |
| **B2** | Отправка заказа в 1С | `SendOrderJob`: после создания заказа выставляет **`sync_status = SYNCED`** без HTTP-вызова; комментарий `// todo real send` | Реальный `POST /post_orders`; статус по ответу 1С; ошибки — retry / FAILED | `SendOrderJob.php` |
| **B4** | Безопасность exchange | `ExchangeController` принимает payload **без** проверки HMAC и IP allowlist | ЧТЗ 09: `X-Palizh-Signature`, allowlist (#48) | `ExchangeController`, middleware |
| **—** | Регистрация «Стать клиентом» | `SendRegistrationRequestJob`: `// todo real send` — заявка не уходит получателю | ЧТЗ 05: доставка заявки менеджеру / в 1С | `SendRegistrationRequestJob.php` |

**Приёмка после исправления B5:** чек-лист [`Приёмка_корзины_vs_счёт_1С.md`](Приёмка_корзины_vs_счёт_1С.md).
...
| **08** | 08 | Поиск по списку заказов | `?search=`, `?status=` в `OrderController::index` | **Частично** | API готов; UI нет; ЧТЗ 08 §4.1.1 |
| **02** | 02 | Документы в ЛК | `Document` model + `sync.documents` | **Частично** | Контракт `GET /documents` в public-http; роуты — **нет** |
...
Источник: [`Реестр_открытых_вопросов.md`](../Интервью%20и%20встречи/Реестр_открытых_вопросов.md). Влияют на сроки бэкенда и приёмку MVP.

| № | Тема | Блокер / задача | Статус |
|---|------|-----------------|--------|
| **18** | HTTP-сервисы 1С: схемы заказов, статусов, документов | **B1**, **B3**, **B6** | Открыт |
| **37** | Идемпотентность outbound, связка GUID заказа | **B1**, **B2**, W1 | Открыт |
| **47** | Тестовая 1С, доступы, стенд интеграции | **B1–B3**, **B6** | Открыт |
| **48** | Auth приложения к 1С (после IP allowlist) | **B1**, **B4** | Открыт |
| **49** | Контракт передачи файлов документов из 1С | **B6** | Открыт |
| **36** | Какие TDS / MSDS / СГР заводить в 1С для ЛК | **B6** | Открыт |
| **40** | Поле архивности номенклатуры для «повтор заказа» | Заказы API, **B3** | Открыт |
| **43** | Редактирование контактов компании клиентом | Профиль API, W5 | Открыт |
| **50** | НФТ: RPO/RTO, 152-ФЗ | Prod, **B4** | Открыт |
| **54** | Заполнение атрибутов в 1С | Качество каталога / поиска | Открыт |
| **34** | Поиск: номенклатура контрагента | W4 (страница поиска) | **Частично закрыт** — suggest `GET /search`; post-MVP фильтр |
| **57** | Две цены / mixed qty в корзине | **B5** | **Закрыт в доках**; в коде — нет |
...
## 6. Чек-лист приёмки MVP (бэкенд)

Высокий уровень; полный сквозной чек-лист — в [`Аудит_готовности_2026-07-06.md`](../Аудит_готовности_2026-07-06.md) §6.

- [ ] **B1–B2:** заказ из API уходит в 1С (`post_orders`); `sync_status` отражает реальный ответ, не заглушка
- [ ] **B3:** inbound обновляет статусы заказа и блок доставки в ЛК
- [ ] **B4:** `/exchange` — HMAC + IP allowlist; ответ по контракту
- [ ] **B5:** `CartItem` и `lineTotal` по ЧТЗ 01 §4.2.4; прогон [`Приёмка_корзины_vs_счёт_1С.md`](Приёмка_корзины_vs_счёт_1С.md)
- [ ] **B6:** API документов ЛК + `get_documents`; файлы по №49
- [ ] **W2:** `integrationSyncState` в ответах заказа
- [ ] **W3:** email по ключевым событиям ЧТЗ 10 (заказ, claim, обучение)
- [ ] **W5:** RBAC по разделам ЛК (ЧТЗ 07 §4.6)
- [ ] Smoke-тесты по `public-http-openapi.yaml`

---

**См. также:** [`Frontend_готовность_MVP_матрица.md`](Frontend_готовность_MVP_матрица.md), сводный [`Аудит_готовности_2026-07-06.md`](../Аудит_готовности_2026-07-06.md).
