...
**Обучение (ЧТЗ 14):** программы и заявки — контент **админки/платформы**; **обмена с 1С нет** (см. `openapi_mvp_полный_черновик.yaml`, тег `Training` — **не** в `openapi_client_mvp.yaml`).
...
| ----- | ---- | ------------------------ | ----- | ---- |
| `POST` | `/exchange` | `sync.products` | Каталог: номенклатура, варианты, цены, остатки, атрибуты, картинки (по примерам `входящие/incoming-hooks.md`). | **MVP-1 (каталог)** |
| `POST` | `/exchange` | `sync.categories` | Дерево категорий, привязка к коду/пути каталога. | **MVP-1 (каталог)** |
...
**Post-MVP (не в YAML, при необходимости — только в описаниях/отдельном согласовании):** резервный опрос справочников в 1С (`GET` категорий/номенклатуры) для отладки или сверки **не** заменяет импорт через **`POST /exchange`**. Раньше такие пути фигурировали в черновиках; в текущем `1c-http-openapi.yaml` их **нет**.
...
| *Post-MVP* (`openapi_mvp_полный_черновик.yaml`) | Претензии, админка, обучение и др. | Обучение — **без 1С**; претензии/прочее — по соответствующим ЧТЗ |
...
| **1a** | `POST /exchange` (`sync.products`, `sync.categories`, `sync.counterparties`), full + delta, публичные `GET` каталога | Витрина с актуальными товарами/категориями |
...
2. **1С-разработчик:** реализовать **приоритетно** `POST /exchange` на платформу (три `sync.*` и дельты) + при необходимости регламент/события в 1С для **дельт**; затем **POST** `/post_orders` в сторону 1С.
...
*Дата актуализации: по согласованию в проекте (каталог push из 1С; обучение не из 1С).*
